...
#gbm = xgb.XGBRegressor(params).fit(X_train,Y_train)
#
#Y_pred_xgboost = gbm.predict(X_train)
#in_sample_rmse_xgboost = mean_squared_error(Y_train, Y_pred_xgboost)**0.5 
#
#Y_predval_xgboost = gbm.predict(X_val)
...
######################## Hyperparameter tuning using Hyperopot #################################
...
Y_train_log = np.array(Y_train).astype('str')
Y_val_log = np.array(Y_val).astype('str')

model_logreg = LogisticRegression( penalty="l2", dual=False, fit_intercept=True,\
                 intercept_scaling=1.0,class_weight = 'balanced')
model_logreg.fit(X_train,Y_train_log)
prob = model_logreg.predict_proba(X_val)
Prob_insample = model_logreg.predict_proba(X_train)
pred_label = model_logreg.predict(X_val)

pred_label = pred_label.astype(float) 
rmse_log = mean_squared_error(Y_val,pred_label)**0.5

######################################################################################


##################### create result data frame
columns1 = ["1" ,"1.25", "1.33", "1.5","1.67", "1.75", "2","2.25" ,"2.33", "2.5","2.67","2.75","3"]
prob_df =  pd.DataFrame(data=prob[0:,0:],   
                        columns=columns1)
prob_df.loc[:,'Actual_Lables'] = Y_val
prob_df.loc[:,'Pred_rf'] = Y_predval


################## create data frame for further training 
columns1 = ["1" ,"1.25", "1.33", "1.5","1.67", "1.75", "2","2.25" ,"2.33", "2.5","2.67","2.75","3"]
prob_train_df =  pd.DataFrame(data=Prob_insample[0:,0:],   
                        columns=columns1)
prob_train_df.loc[:,'Actual_Lables'] = Y_train



################## create data frame for validation data set
columns1 = ["1" ,"1.25", "1.33", "1.5","1.67", "1.75", "2","2.25" ,"2.33", "2.5","2.67","2.75","3"]
val_df =  pd.DataFrame(data=prob[0:,0:],   
                        columns=columns1)
val_df.loc[:,'Y_val'] = Y_val
...
cosine_similarities = linear_kernel(features_search_term[0:1], features_prodc_title[0:1])
cosine_similarities2 = linear_kernel(features_search_term[0:1], features_prodc_desc[0:1])

all_features[0:1]



SnowballStemmer('english').stem('shelves')

strng= " Spray distance from 8 ft. to 15 ft.  "

      
words =  process_strings(strng)          
   
words = re.sub('(\w+)([A-Z][a-z]+)',lambda m: m.group(0) + " " + m.group(1) +\
               " " + m.group(2), strng )


words = re.sub('\b[a-z0-9]+\b', 'f', strng )


strng2 = ["dwddfd"]
strng2.append(train_proddesc["product_title"][3])


         

string = BeautifulSoup(strng)
words = string.get_text()

words = re.sub("[^a-zA-Z0-9]"," ",words )    
stemmed_words = PorterStemmer().stem_word(words)



ddd = re.sub('[^a-zA-Z0-9\s]', " ", strng )
ddd2 = re.sub( "(\d+)x(\d+)", lambda m: m.group(1) + " " + m.group(2)  , ddd )
...
fff = re.sub( "(\D+)(\d+)", lambda m:  m.group(1) + " " + m.group(2), strng ) 

fff2 = re.sub( "(\d+)(\D+)", lambda m:  m.group(1) + " " + m.group(2), fff )

fff3 = re.sub( "(\d+)(\D+)(\d+)", lambda m:  m.group(1) + " " + m.group(2) + " " + m.group(3), fff2)
